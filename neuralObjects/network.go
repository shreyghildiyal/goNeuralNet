...
func NewNetwork(inputLayerNodeCount int, interLayersNodeCounts []int, outputLayerNodeCount int) Network {
	network := Network{}
	network.inputLayer = NewNeuralLayer(inputLayerNodeCount, 0, activation.FastSigmoid)

	prevNodeCount := inputLayerNodeCount

	for _, internalLayerNodeCount := range interLayersNodeCounts {
		network.internalLayers = append(network.internalLayers, NewNeuralLayer(internalLayerNodeCount, prevNodeCount, activation.FastSigmoid))
